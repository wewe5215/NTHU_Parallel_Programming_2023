...
__global__ void Phase3(int* d, int round, int V, int Yoffset){
    // put data to shared memory
    // printf("blockIdx.x = %d, blockIdx.y = %d\n", blockIdx.x, blockIdx.y);
    if(round == blockIdx.x || round == blockIdx.y + Yoffset)return;
    __shared__ int pivot[blocksize][blocksize];
    __shared__ int row[blocksize][blocksize];
    __shared__ int col[blocksize][blocksize];
    int i = threadIdx.x;
    ...
    int idx_y_col = j + (blockIdx.y + Yoffset) * blocksize; //x is fixed, only y change
    // pivot
...
int main(int argc, char** argv) {
    struct timespec start, end, comp_tmp1, comp_tmp2;
    ...
    int half_total = total_rnd / 2;
    dim3 grid_ph2(1, total_rnd);
    double comp = 0;
    #pragma omp parallel num_threads(2)
    {
        int tid = omp_get_thread_num();
        int data_to_solve = (tid) ? (total_rnd - half_total) : half_total;
        int Yoffset = (tid) ? half_total : 0;
        dim3 grid_ph3(total_rnd, data_to_solve);
        cudaSetDevice(tid);
        cudaMalloc(&device_d[tid], d_size);
        cudaMemcpy(device_d[tid], d, d_size, cudaMemcpyHostToDevice);
        int tmp = blocksize * V;
        for(int k = 0; k < total_rnd; k ++){

            if(k >= Yoffset && k < Yoffset + data_to_solve){
                cudaMemcpy(device_d[!tid] + k * tmp, device_d[tid] + k * tmp, tmp * sizeof(int), cudaMemcpyDeviceToDevice);
            }

            #pragma omp barrier

            clock_gettime(CLOCK_MONOTONIC, &comp_tmp1);
            //dot
            Phase1 << <1, blk>>> (device_d[tid], k, V);
            //line
            Phase2 << <grid_ph2, blk>>>(device_d[tid], k, V);
            //plane
            Phase3 << <grid_ph3, blk>>>(device_d[tid], k, V, Yoffset);
            clock_gettime(CLOCK_MONOTONIC, &comp_tmp2);
            comp += ((comp_tmp2.tv_sec - comp_tmp1.tv_sec) + (comp_tmp2.tv_nsec - comp_tmp1.tv_nsec) / 1e9);
        }
        cudaMemcpy(d + Yoffset * tmp, device_d[tid] + Yoffset * tmp, data_to_solve * tmp * sizeof(int), cudaMemcpyDeviceToHost);
    }
