...
__global__ void Phase1(int* d, int round, int V){
    ...
    shared[j][i] = d[idx_d];
    shared[j + 4][i] = d[idx_d + V * 4];
    shared[j][i + 4] = d[idx_d + 4];
    shared[j + 4][i + 4] = d[idx_d + V * 4 + 4];
    __syncthreads();
    // calculation
    #pragma unroll
    for(int k = 0; k < blocksize; k ++){
        shared[j][i] = min(shared[j][i], shared[j][k] + shared[k][i]);
        shared[j + 4][i] = min(shared[j + 4][i], shared[j + 4][k] + shared[k][i]);
        shared[j][i + 4] = min(shared[j][i + 4], shared[j][k] + shared[k][i + 4]);
        shared[j + 4][i + 4] = min(shared[j + 4][i + 4], shared[j + 4][k] + shared[k][i + 4]);
        __syncthreads();
    }

    //write back
    d[idx_d] = shared[j][i];
    d[idx_d + V * 4] = shared[j + 4][i];
    d[idx_d + 4] = shared[j][i + 4];
    d[idx_d + V * 4 + 4] = shared[j + 4][i + 4];
}
...
__global__ void Phase2(int* d, int round, int V){
    ...
    int idx_d = idx_y * V + idx_x;
    pivot[j][i] = d[idx_d];
    pivot[j + 4][i] = d[idx_d + V * 4];
    pivot[j][i + 4] = d[idx_d + 4];
    pivot[j + 4][i + 4] = d[idx_d + V * 4 + 4];
    //row --> y fixed
    int idx_row_d = idx_y * V + idx_x_row;
    row[j][i] = d[idx_row_d];
    row[j + 4][i] = d[idx_row_d + V * 4];
    row[j][i + 4] = d[idx_row_d + 4];
    row[j + 4][i + 4] = d[idx_row_d + V * 4 + 4];
    //col --> x fixed
    int idx_col_d = idx_y_col * V + idx_x;
    col[j][i] = d[idx_col_d];
    col[j + 4][i] = d[idx_col_d + V * 4];
    col[j][i + 4] = d[idx_col_d + 4];
    col[j + 4][i + 4] = d[idx_col_d + V * 4 + 4];
    __syncthreads();
    #pragma unroll
    for(int k = 0; k < blocksize; k ++){
        row[j][i] = min(row[j][i], pivot[j][k] + row[k][i]);
        row[j + 4][i] = min(row[j + 4][i], pivot[j + 4][k] + row[k][i]);
        row[j][i + 4] = min(row[j][i + 4], pivot[j][k] + row[k][i + 4]);
        row[j + 4][i + 4] = min(row[j + 4][i + 4], pivot[j + 4][k] + row[k][i + 4]);

        col[j][i] = min(col[j][i], col[j][k] + pivot[k][i]);
        col[j + 4][i] = min(col[j + 4][i], col[j + 4][k] + pivot[k][i]);
        col[j][i + 4] = min(col[j][i + 4], col[j][k] + pivot[k][i + 4]);
        col[j + 4][i + 4] = min(col[j + 4][i + 4], col[j + 4][k] + pivot[k][i + 4]);
        __syncthreads();
    }

    //row --> y fixed
    d[idx_row_d] = row[j][i];
    d[idx_row_d + V * 4] = row[j + 4][i];
    d[idx_row_d + 4] = row[j][i + 4];
    d[idx_row_d + V * 4 + 4] = row[j + 4][i + 4];
    //col --> x fixed
    d[idx_col_d] = col[j][i];
    d[idx_col_d + V * 4] = col[j + 4][i];
    d[idx_col_d + 4] = col[j][i + 4];
    d[idx_col_d + V * 4 + 4] = col[j + 4][i + 4];
}
...
__global__ void Phase3(int* d, int round, int V){
    ...
    for(int k = 0; k < blocksize; k ++){
        ...
        pivot[j + 4][i] = min(pivot[j + 4][i], row[j + 4][k] + col[k][i]);
        pivot[j][i + 4] = min(pivot[j][i + 4], row[j][k] + col[k][i + 4]);
        pivot[j + 4][i + 4] = min(pivot[j + 4][i + 4], row[j + 4][k] + col[k][i + 4]);
    }

    d[idx_d] = pivot[j][i];
    d[idx_d + V * 4] = pivot[j + 4][i];
    d[idx_d + 4] = pivot[j][i + 4];
    d[idx_d + V * 4 + 4] = pivot[j + 4][i + 4];
}
