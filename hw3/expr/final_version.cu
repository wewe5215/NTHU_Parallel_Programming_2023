...
int main(int argc, char** argv) {
    ...
    struct timespec start, end, IO_tmp, mem_tmp1, mem_tmp2;
    clock_gettime(CLOCK_MONOTONIC, &start);
    handle_input(argv[1]);
    clock_gettime(CLOCK_MONOTONIC, &IO_tmp);
    double IO_total = (IO_tmp.tv_sec - start.tv_sec) + (IO_tmp.tv_nsec - start.tv_nsec) / 1e9;
    // for GPU code
    ...
    clock_gettime(CLOCK_MONOTONIC, &mem_tmp1);
    cudaMemcpy(device_d, d, d_size, cudaMemcpyHostToDevice);
    clock_gettime(CLOCK_MONOTONIC, &mem_tmp2);
    double Mem_total = (mem_tmp2.tv_sec - mem_tmp1.tv_sec) + (mem_tmp2.tv_nsec - mem_tmp1.tv_nsec) / 1e9;
    dim3 blk(32, 32);
    ...
    clock_gettime(CLOCK_MONOTONIC, &mem_tmp2);
    Mem_total += ((mem_tmp2.tv_sec - mem_tmp1.tv_sec) + (mem_tmp2.tv_nsec - mem_tmp1.tv_nsec) / 1e9);
    clock_gettime(CLOCK_MONOTONIC, &IO_tmp);
    handle_output(argv[2]);
    clock_gettime(CLOCK_MONOTONIC, &end);
    IO_total += ((end.tv_sec - IO_tmp.tv_sec) + (end.tv_nsec - IO_tmp.tv_nsec) / 1e9);
    double elapsed = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
    printf("vertex num = %d\n", original_V);
    printf("Elapsed time: %.9f seconds\n", elapsed);
    printf("Memcpy = %.9f, I/O = %.9f, computing = %.9f\n", Mem_total, IO_total, elapsed - IO_total - Mem_total);
    return 0;
}
