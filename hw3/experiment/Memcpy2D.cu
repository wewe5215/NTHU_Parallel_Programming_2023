...
int main(int argc, char** argv) {
    ...
    size_t pitch, s_pitch = V * sizeof(int);
    cudaMallocPitch((void**)& device_d, (size_t*) &pitch, s_pitch, V);
    cudaMemcpy2D(device_d, pitch, d, s_pitch, s_pitch, V, cudaMemcpyHostToDevice);
    ...
    for(int k = 0; k < total_rnd; k ++){
        size_t p = pitch / sizeof(int);
        //dot
        Phase1 << <1, blk>>> (device_d, k, p);
        //line
        Phase2 << <grid_ph2, blk>>>(device_d, k, p);
        //plane
        Phase3 << <grid_ph3, blk>>>(device_d, k, p);
    }

    cudaMemcpy2D(d, s_pitch, device_d, pitch, s_pitch, V, cudaMemcpyDeviceToHost);
