...
int main(int argc, char** argv) {
    ...
    long long int size = height * width * channels * sizeof(unsigned char);
    unsigned char* host_t = (unsigned char*) malloc(size);
    
    /* Hint 1 */
    // cudaMalloc(...) for device src and device dst
    unsigned char* device_src, *device_dst;
    
    cudaMalloc((void **) &device_src, size);
    cudaMalloc((void **) &device_dst, size);
    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    cudaMemcpy(device_src, host_s, size, cudaMemcpyHostToDevice);
    /* Hint 3 */
    // acclerate this function
    dim3 threadsPerBlock(16, 16);
    dim3 numBlocks(ceil(width / float(threadsPerBlock.x)), ceil(height / float(threadsPerBlock.y)));
    sobel<<<numBlocks, threadsPerBlock>>>(device_src, device_dst, height, width, channels);
    
    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, device_dst, size, cudaMemcpyDeviceToHost);
